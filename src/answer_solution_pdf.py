...
def render_answer_solution_pdf(paper, identity, path):
    ...
    for a in answer_entries(paper):
        story.append(Paragraph(f"<b>{a.number}.</b> {escape(humanize_math(a.answer))}", styles["BodyText"]))
    story += [
        PageBreak(),
    ...
    for a in answer_entries(paper):
        story.append(KeepTogether([
            ...
            Paragraph(escape(humanize_math(a.solution or "Detailed solution not supplied.")), styles["BodyText"]),
            Spacer(1, 4 * mm),
        ]))
